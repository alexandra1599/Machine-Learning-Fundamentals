...
def new_eval_learning_alg(learner, data_gen, n_train, n_test, it):
    '''
    Evaluate the learner's performance on the training data through multiple iterations.

    Parameters:
    - learner: function that trains a model and returns the learned parameters (th, th0)
    - data_gen: function that generates data and labels, returns a tuple (data, labels)
    - n_train: the size of the training set
    - n_test: the size of the test set (not used here, since we are evaluating on the training data)
    - it: the number of iterations to average over

    Returns:
    - average_training_accuracy: The average classification accuracy on the training set over multiple iterations (float between 0 and 1)
    '''
    training_accuracies = []
    ...
    for _ in range(it):
        ...
        # Step 2: Train the model using the learner function
        th, th0 = learner(data_train, labels_train)

        # Step 3: Evaluate the model's accuracy on the training data (training accuracy)
        training_accuracy = score(data_train, labels_train, th, th0) / n_train
        
        # Step 4: Append the accuracy for this iteration
        training_accuracies.append(training_accuracy)

    # Step 5: Calculate the average training accuracy
    average_training_accuracy = np.mean(training_accuracies)
    
    # Return the average training accuracy as a float between 0 and 1
    return average_training_accuracy
...
print(new_eval_learning_alg(averaged_perceptron, gen_flipped_lin_separable(pflip=.25), 20, 20, 5))
